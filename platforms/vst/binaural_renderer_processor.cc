...
tresult PLUGIN_API BinauralRendererVst::initialize(FUnknown* context) {
  ...
  // Add a single input bus that accepts a discrete arrangement (arbitrary
  // channel count) and a single stereo output bus. Host may reconfigure the
  // input bus arrangement later via setBusArrangements.
...
tresult PLUGIN_API BinauralRendererVst::setBusArrangements(
  ...
  // Accept any input arrangement (discrete/ambisonic). The ambisonic support
  // check is performed later in initBinauralSurroundRenderer based on channel
  // count. Keep the bus arrangement by delegating to base implementation.
...
// -----------------------------------------------------------------------------
tresult PLUGIN_API
BinauralRendererVst::canProcessSampleSize(int32 symbolicSampleSize) {
  return (symbolicSampleSize == SymbolicSampleSizes::kSample32) ? kResultTrue
                                                                : kResultFalse;
}
...
bool areProblematicRates(int destination_frequency, int source_frequency) {
  ...
  const auto destination = static_cast<size_t>(destination_frequency / gcd);
  const auto source = static_cast<size_t>(source_frequency / gcd);
  const auto max_rate = std::max(destination, source);
  const auto cutoff_frequency =
      static_cast<float>(source_frequency) / static_cast<float>(2 * max_rate);
  const auto filter_length_bw = max_rate * kTransitionBandwidthRatio;
  const auto filter_length = filter_length_bw + filter_length_bw % 2;
...
tresult PLUGIN_API BinauralRendererVst::process(ProcessData& data) {
  ...
  if (!active_ || data.numInputs == 0 || data.numOutputs == 0 ||
      data.numSamples <= 0)
    return kResultOk;
  ...
  if (numInputChannels < channels_) return kInvalidArgument;

  if (!binauralSurroundRenderer_)
    if (!initBinauralSurroundRenderer(framesPerBuffer_, channels_,
                                      sampleRateHz_))
      return kNotInitialized;

  // Directly use the channelBuffers32 arrays.
  binauralSurroundRenderer_->AddPlanarInput(inBus.channelBuffers32, channels_,
                                            sampleFrames);
  binauralSurroundRenderer_->GetPlanarStereoOutput(outBus.channelBuffers32,
                                                   sampleFrames);
...
// -----------------------------------------------------------------------------
bool BinauralRendererVst::initBinauralSurroundRenderer(int32 framesPerBuffer,
