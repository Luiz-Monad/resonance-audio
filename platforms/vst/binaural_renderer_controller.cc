...
ChannelsParameter::ChannelsParameter()
    : RangeParameter(STR16("Input Channels"), kParamNumInputChannels,
                     STR16("ch"), 1.0, 64.0, 1.0  // min, max, default
      ) {
  setPrecision(0);
}
ChannelsParameter* Steinberg::Vst::Param_NumInputChannels =
...
// -----------------------------------------------------------------------------
tresult PLUGIN_API
BinauralRendererVstController::initialize(FUnknown* context) {
...
// -----------------------------------------------------------------------------
tresult PLUGIN_API
BinauralRendererVstController::setComponentState(IBStream* state) {
  if (!state) {
    return kResultFalse;
  }
